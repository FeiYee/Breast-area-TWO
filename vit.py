...
for fold_index in range(fold_num):
    ...
    logging.basicConfig(level=logging.INFO,
                    filename = "log_BreaKHis_v1/vit.log",
                    format='[%(asctime)s] - %(message)s')
    ...
    for epoch in range(epochs):
        ...
        if epoch_accuracy_test > best_acc:
            ...
            torch.save(obj=model.state_dict(), f='model_BreaKHis_v1/best_vit_' + str(fold_index) +'.pth')
...
# train_data = datasets.ImageFolder('Dataset_BUSI_with_GT/train', transform=train_transforms)
# test_data = datasets.ImageFolder('Dataset_BUSI_with_GT/test', transform=test_transforms)
...
#         print(best_acc)
#         best_acc = epoch_accuracy_test
#         torch.save(obj=model.state_dict(), f='model/vit.pth')

#     logging.info(
#         f"Epoch : {epoch+1} - loss : {epoch_loss:.4f} - acc: {epoch_accuracy_train:.4f} - test_loss : {epoch_test_loss:.4f} - test_acc: {epoch_accuracy_test:.4f}\n"
#     )
